...
class RegistrarBarGraphController extends GetxController {
  ...
  Future<void> generatePdf(ScreenshotController controller) async {
    ...
    if (image != null) {
      ...
      html.AnchorElement(href: url)
        ..setAttribute(
            "download", "registrar_office_questionnaire_${version.value}.pdf")
        ..click();
  ...
  Future<void> getQuestionsFilter() async {
    ...
    try {
      ...
      _status.value = RegistrarBarGraphControllerStatus.error;
    }
  ...
  Future<void> getUserTotalsFilter(String officeName) async {
    MyLogger.printInfo('getUserTotals');
    users.value = await UserRepository.getUsersRegistrarOfficeAnsweredFilter(
        office: officeName,
